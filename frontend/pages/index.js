...
export default function Home() {
  ...
  const checkNetwork = async () => {
    ...
    if (chainId !== "0x4") {
      ...
      window.alert(
        "This Dapp works on Rinkeby Test Network Only. Please Approve to switch to Rinkeby"
      );
  ...
  const createProposal = async () => {
    try {
      ...
      const txn = await daoContract.createProposal(
        fakeNftTokenId,
        FAKE_NFT_MARKETPLACE_ADDRESS,
        parseEther(costOfToken.toString())
      );
  ...
  const fetchProposalById = async (id) => {
    try {
      ...
      const proposalContract = getProposalContractInstance(
        provider,
        proposal_address
      );
      ...
      const parsedProposal = {
        ...
        priceOfNft: proposal.priceOfNft.toString() + " btc",
  ...
  // given a Provider/Signer
  const getProposalContractInstance = (
    providerOrSigner,
  ...
  return (
    <div>
      <Head>
        ...
        ></link>
        <link
          href="https://fonts.googleapis.com/css2?family=Palette+Mosaic&family=Libre+Baskerville&family=Raleway:wght@600&display=swap"
          rel="stylesheet"
        ></link>
      </Head>
      ...
      <div className={styles.main}>
        <div>
          <h1 className="font-Cinzel font-bold text-5xl pt-8">
            Welcome to the Crisis Chime DAO!
          </h1>
          {/* <div className={styles.description}>Welcome to the DAO!</div> */}
          <div className={styles.description}>
            Your Staked Balance: {nftBalance}
            <br />
